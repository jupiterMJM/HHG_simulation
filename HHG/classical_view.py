...
ionization_potential = 15.76  # eV, NOT IN A.U. the conversion is done later

# parameters of the simulation
time_range = np.linspace(0, 5, 1000)  # time range in fs, NOT IN A.U. the conversion is done later
...
for time_ionization in np.arange(0, time_range[-1], step_ionization):
    ...
    if np.all(x[1:] * x[1] > 0): # if the electron is always on the same side of the ion
        ...
        ax.plot(t * 1e15, x * 1e9, label=f'Ionization at {time_ionization} fs', color='lightblue')  # convert x to nm for plotting
    else:
